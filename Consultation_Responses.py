...
#To-Do:
#the B_question idea currently does not work if the B_column happens after a multicolumn question
#The logic must be revisited.
...
def smartcolumnsfunction(data):
    ...
    for i in range(data.shape[1]):
        if pattern.match(data.columns[i]):
...
for i in range(firstcol,(lastcol+1)):
    if i in B_columns or i in additional_selectallthatapply_columns or i in pleasespecify_columns or i in other_rankcolumns: 
        continue
    ...
    if i in graphcols and selectallthatapplyoptions[i] == "" and not((i+1) in pleasespecify_columns) and not(i in rankcolumns): #this excludes select all that apply questions
        smartplotfunction(data[[data.columns[i]]],i, data)
        
    
    if i in graphcols: #this excludes select all that apply questions
